<template>
  <div class="container-car">
    ...
    <div class="flex" @click="moveInfo($props.item.itemId)">
      ...
      >
        <div>
          <span style="color:rgb(163, 163, 163); font-size:12px">总计:</span>
          <span style="color:red;font-weight:bold"
            >￥{{
              (
                $props.item.price * $props.item.num || $props.item.priceTotal
              ).toFixed(2)
            }}</span
          >
        </div>
...
export default {
  ...
  methods: {
    ...
    edit(cId, itemId, delPrice) {
      ...
      this.$http.delete(`/shop/Trolley/${cId}/${itemId}`);
    },
    goodsBtn(item) {
      // console.log()
