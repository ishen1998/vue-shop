<template>
  <div>
    ...
    <van-swipe :autoplay="3000" indicator-color="white">
      <van-swipe-item v-for="item in activity.returnImg" :key="item.id">
        <img
          @click="getSortList(item.id)"
          :src="item.img"
          width="100%"
          height="200"
        />
    ...
    <div class="activity">
      ...
      <van-row>
        <van-col
          v-for="item in activity.activityImg"
          :key="item.activityId"
          span="12"
        >
          <div>
            <img
              @click="getActivity(item.activityId)"
        ...
        >
          ...
          >
            <img
              ...
              width="100%"
              height="100px"
              v-lazy="item.activityInfo.activityImg"
            />
            <!-- 活动轮播图 -->
            <van-swipe
              ...
              >
                <van-row>
                  ...
                  >
                    <img
                      ...
                      "
                      v-lazy="huoDong[index].itemInfos[i - 1][num - 1].coverImg"
                      alt="图片加载失败"
                    />
                  </van-col>
                </van-row>
                <van-row>
                  ...
                  >
                    <span
                      >{{
                        huoDong[index].itemInfos[i - 1][num - 1].price
                      }}元</span
                    >
                  </van-col>
                </van-row>
...
export default {
  ...
  methods: {
    ...
    Activity(name) {
      ...
      this.$http
        ...
        .then(res => {
          console.log("活动：", res.data.data.HomeActivity);
  ...
  created() {
    ...
    this.$http
      ...
      .then(res => {
        console.log("轮播超值：", res.data.data);
    ...
    this.$http
      ...
      .then(res => {
        console.log("Tab页", res.data.data.HomeActivity);
        this.huoDong = res.data.data.HomeActivity;
