<template>
  <div>
    ...
    <van-tabs @click="upDataTicket" v-model="active">
      <van-tab title="全部卡劵">
        <div
          class="kajuan"
        ...
        >
          <van-panel
            ...
            :desc="
              item.ticketRule
                ? '满' +
                  item.ticketRule.split(',')[0] +
                  '减' +
                  item.ticketRule.split(',')[1]
                : item.ticketRule
            "
          ...
          >
            ...
            <div slot="footer">
              <van-button
                :disabled="item.ticketNum ? false : true || item.status == 'J'"
        ...
        >
          <van-cell class="kajuan">
            <van-panel
              ...
              :desc="
                item.ticketInfo.ticketRule
                  ? '满' +
                    item.ticketInfo.ticketRule.split(',')[0] +
                    '减' +
                    item.ticketInfo.ticketRule.split(',')[1]
                  : item.ticketInfo.ticketRule
              "
      ...
      <van-tab title="已过期">
        <van-swipe-cell
          v-show="item.useStatus == '3' ? true : false"
          v-for="(item, index) in kList"
          :key="index"
        >
          <van-cell class="kajuan">
...
export default {
  ...
  methods: {
    ...
    deTicket(id) {
      console.log(id);
      this.$http
        .get("/shop/ticket/deleteCustomerTicket?id=" + id)
        .then(res => {
          console.log(res);
          this.getkajun();
          this.$toast("卡劵已删除！");
    ...
    getAllTicket() {
      this.$http
        .get("/shop/ticket/selectAllTicket")
        .then(res => {
          console.log(res);
          this.kajuan = res.data.data;
          // this.kajuan.forEach(item => {
          //   console.log(item.ticketRule.split(","));
          // });
